...
| **Endpoint de depuración local** sobre el canal de control ya existente (§11.4) — no una superficie nueva, una ruta más (`GET /debug`) en el mismo `control.Server`, protegida por el mismo transporte endurecido en §11.7. Contenido: goroutines activas, `runtime.MemStats` del propio proceso, y las últimas N líneas de log en memoria (un ring buffer pequeño de texto, no de payloads). Pensado para un futuro `hygeia-agent debug` que se conecta al canal y vuelca ese estado por consola — diagnóstico de campo sin depender de encontrar el fichero de log. | No | ✅ *(incluido el subcomando `hygeia-agent debug`, no quedó solo como plomería)* |
